		...
		const templates = {
			service: item => {
				const imgSection = Array.isArray(item.images)
					? `<div class="circle-grid">${item.images.map(src => `<img src="${imgPath(src)}" alt="" />`).join('')}</div>`
					: `<a href="#" class="image"><img src="${imgPath(item.image)}" alt="${item.alt}" data-position="${item.position || 'center center'}" /></a>`;
				return `
				<section>
					${imgSection}
					<div class="content">
